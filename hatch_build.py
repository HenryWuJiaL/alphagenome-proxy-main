...
class GenerateProtos(BuildHookInterface):
  ...
  def initialize(self, version: str, build_data: dict[str, Any]) -> None:
    ...
    # This is the corrected part: build a single command with all proto files
    proto_args = [
        'grpc_tools.protoc',
    ...
    # Add all proto files to the command arguments
    for proto_path in _ALPHAGENOME_PROTOS:
      proto_args.append(os.path.join(_ROOT_DIR, proto_path))

    # Run the command once with all files included
    if protoc.main(proto_args) != 0:
      raise RuntimeError(f'ERROR: protoc command failed with args: {proto_args}')